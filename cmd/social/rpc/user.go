...
func initUser() {
	...
	provider.NewOpenTelemetryProvider(
		provider.WithServiceName(consts.SocialServiceAddr),
	...
	c, err := userservice.NewClient(
		...
		client.WithClientBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: consts.SocialServiceAddr}),
	)
	if err != nil {
		panic(err)
	}
...
func GetUser(ctx context.Context, req *user.GetUserRequest) (*user.User, error) {
	...
	if err != nil || resp.StatusCode != errno.Success.ErrCode {
		klog.Error("social get user error")
